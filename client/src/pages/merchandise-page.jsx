...
export default function MerchandisePage() {
  ...
  // Status options
  const statusOptions = ["In Stock", "Low Stock", "Out of Stock", "New Arrival", "Seasonal", "Discontinued"];
  
  // Fetch merchandise
  const { data: merchandise = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      try {
        ...
        if (!res.ok) {
          console.error("Failed to fetch merchandise:", res.status);
          return [];
        }
  ...
  // Filter merchandise based on search term and status
  ...
  // Handle create/edit merchandise
  ...
  const handleDateChange = (dateString) => {
    setCurrentMerchandise(prev => ({
      ...prev,
      dateAdded: new Date(dateString)
    }));
  };
  
  // Handle save merchandise
  const handleSaveMerchandise = async () => {
    try {
      const url = isCreating ? "/api/merchandise" : `/api/merchandise/${currentMerchandise.id}`;
      const method = isCreating ? "POST" : "PUT";
      
      // Format dates for API
      const merchandiseData = {
        ...currentMerchandise,
        dateAdded: currentMerchandise.dateAdded ? currentMerchandise.dateAdded.toISOString() : null
      };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold mb-2">Merchandise</h1>
        <p className="text-gray-600 mb-6">
          Manage all cafe merchandise and retail items
        </p>
      </div>
      ...
      {/* Merchandise Edit/Create Modal */}
      <Modal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        title={isCreating ? "Add New Merchandise" : "Edit Merchandise"}
        footer={
          <>
            <Button variant="outline" onClick={() => setIsModalOpen(false)}>
              Cancel
            </Button>
            <Button onClick={handleSaveMerchandise}>
              Save
            </Button>
          </>
        }
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  step="1"
                  value={currentMerchandise.quantity}
                  onChange={handleInputChange}
              ...
              <div className="space-y-2">
                ...
                <select
                  id="status"
                  name="status"
                  value={currentMerchandise.status}
                  onChange={handleInputChange}
                  className="flex h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#6B4226]"
                >
                  {statusOptions.map((status, index) => (
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                name="dateAdded"
                type="date"
                value={currentMerchandise.dateAdded 
                  ? format(new Date(currentMerchandise.dateAdded), "yyyy-MM-dd") 
                  : ""
                }
